...
# load F matrix when starting pipeline at localization steps 1 or 2
parser.add_argument("-load_F", "--load_F", dest = "F_in", default = None, help="load F; for use when", type = str)
...
if first_run_glmnet_needed:
    ...
    F,first_o_preds = mlg.get_coefs(Xtrain,Ytrain,Xval,Yval)
    np.save(open(args.output_prefix+f"/first_F_cc_{args.ct}.npy", 'wb'), F)
    np.save(open(args.output_prefix+f"/first_preds_cc_{args.ct}.npy", 'wb'), first_o_preds)
...
if args.sl2o==False:
    ...
    idx_filt, loci_filt, F_ori, X, Y, Ysub, loci = lc.prep_arrays(F, GT, PT, loci_to_keep, args.lt1, args.v)
...
if args.sl2o == False and args.sl1o== False:
    ...
    np.save(open(args.output_prefix+f"/second_F_cc_{args.ct}_lt1_{args.lt1}_width_{args.width}_std_{args.std}.npy", 'wb'), F)
    np.save(open(args.output_prefix+f"/second_preds_cc_{args.ct}_lt1_{args.lt1}_width_{args.width}_std_{args.std}.npy", 'wb'), preds)
...
print(F.shape)
print(args.lt2)
idx_filt, loci_filt, F_ori, X, Y, Ysub, loci = lc.prep_arrays(F, GT, PT, loci_to_keep, args.lt2, args.v)
...
pickle.dump((loci_localized, all_confidence_intervals), open(args.output_prefix+f"/CI_after_localization_round_2_cc_{args.ct}_lt2_{args.lt2}_width_{args.width}_std_{args.std}.pickle", 'wb'))
